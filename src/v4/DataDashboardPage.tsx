...
export function DataDashboardPage({ snapshot }: { snapshot: Snapshot }) {
  ...
  return <section className="data-dashboard-v41">
    ...
    {tab === "execution" ? <>
      ...
      <section className="analytics-panel-v41"><SectionHeader eyebrow="DXX CANÔNICO" title="Planejamento × execução" detail="Você pode preencher esses campos direto no Notion; o site apenas relê." />
        <div className="execution-cards-mobile-v5">{filteredDays.map((day) => {
          const raw = dayMap.get(day.dxx); const meta = raw?.metaQuestions ?? questionMetaMap.get(day.dxx)?.meta ?? (day.questionSlug ? snapshot.questions[day.questionSlug]?.meta ?? snapshot.questions[day.questionSlug]?.valid : null); const dayIssues = issues.filter((x) => x.source === "Dxx" && x.key === day.dxx && x.level !== "info");
          return <article key={day.dxx} className={raw && hasExecution(raw) ? "has-execution" : ""}>
            <header><a href={href("/dia/" + day.dxx.toLowerCase() + "/")}><strong>{day.session} · {day.dxx}</strong><small>{day.type}</small></a>{dayIssues.length ? <StatusPill tone={dayIssues.some((x) => x.level === "error") ? "danger" : "warning"}>{dayIssues.length} alerta(s)</StatusPill> : raw && hasExecution(raw) ? <StatusPill tone="success">Completo</StatusPill> : <StatusPill>Não executado</StatusPill>}</header>
            <div><span>Tempo<b>{raw?.timeMinutes == null ? "—" : formatMinutes(raw.timeMinutes)}</b></span><span>Questões<b>{raw?.questionsDone == null ? "—" : raw.questionsDone}{meta != null ? " / " + meta : ""}</b></span><span>Precisão<b>{pct(accuracy(raw?.correct, raw?.errors), 1)}</b></span><span>Dúvidas<b>{raw?.doubts ?? "—"}</b></span></div>
          </article>;
        })}</div>
        <div className="analytics-table-wrap-v41 execution-table-desktop-v5"><table className="analytics-table-v41"><thead><tr><th>Sessão</th><th>Tipo</th><th>Origem</th><th>Tempo</th><th>Questões</th><th>Acertos</th><th>Erros</th><th>Dúvidas</th><th>Precisão</th><th>Dados</th></tr></thead><tbody>{filteredDays.map((day) => {
